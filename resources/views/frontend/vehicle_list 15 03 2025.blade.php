                                ...
                                <div class="accordion" id="accordionPanelsStayOpenExample">
                                    @foreach ($vehicles as $bus)
                                        <div class="accordion-item border-0">
                                            ...
                                            {{-- Loop through multiple time tables --}}
                                            @foreach ($vehicleData[$bus->id]['timetables'] as $timetable)
                                                <div class="accordion-header shadow mb-4 mt-0 p-1">
                                                    <div class="row pt-3 px-4">
                                                        ...
                                                        <div class="col-lg-2">
                                                            <h5>{{ calculate_duration($timetable->departure_time, $timetable->arrival_time) }}</h5>
                                                        </div>
                                                        <div class="col-lg-2">
                                                            <h5>{{-- format_time($timetable->arrival_time) --}}{{ format_time($tostoppage->time) }}</h5>
                                                            <p>To: {{ $to }}</p>
                                                        </div>
                                                        <div class="col-lg-1">
                                                            <h5>INR {{ $vehicleData[$bus->id]['fare'] ?? 'N/A' }}</h5>
                            ...
                            for (let i = 0; i < numRows; i++) {
                                ...
                                for (let j = 0; j < numColumns; j++) {
                                    ...
                                    if (seat) {
                                        ...
                                        const seatDiv = $(
                                            `<div class="${isBookable} seat ${seatClass}" data-seat-number="${seat.seat_number}"  data-singleVehicle-id="${vehicleId}" data-time-table-id="${timeTableId}">
            ...
            $(document).on('click', '.get-seat-number', function() {
                ...
                if (seatNumber && vehicleId) {
                    var bookingUrl = "{{ route('home.seat.booking', ':vehicleId') }}".replace(':vehicleId',
                        vehicleId);
                    bookingUrl += `?seat_number=${seatNumber}&timeid=${timeTableId}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${encodeURIComponent(date)}`;

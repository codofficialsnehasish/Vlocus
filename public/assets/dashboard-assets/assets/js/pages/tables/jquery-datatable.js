$(function () {
    ...
    //                     // Add "S.l" as the first column in the header row
    //                     headerRow.unshift('S.l');
    //                     csvRows[0] = headerRow.join(',');

    //                     // Add serial numbers to each subsequent row
    //                     for (let i = 1; i < csvRows.length; i++) {
    //                         const row = csvRows[i].split(',');
    //                         row.unshift(i); // Add serial number as the first column
    //                         csvRows[i] = row.join(',');
    //                     }

    //                     // Prepend title and date to the CSV
    ...
    $('.customize-export').each(function () {
        ...
        $table.DataTable({
            ...
            columnDefs: [
                {
                    targets: 0, // Target the "S.l" column
                    type: 'num', // Set sorting type to numeric
                    orderable: true // Ensure column is sortable
                }
            ],
            order: [[0, 'asc']] // Default sorting by "S.l" in ascending order
        });
    });
